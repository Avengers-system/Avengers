	...
	function fn_search() {
		...
		jQuery("form[name=frm]").attr("action", "").submit();
					...
					<div class="row">
						...
						<form
							action="<%=request.getContextPath()%>/admin/qnaProfSearch?board_title=${qnaProfSearch.board_title}">
							<div
								style="text-align: right; margin-right: 28px; margin-top: 13px; margin-bottom: -20px;">

								<a href="<%=request.getContextPath()%>/admin/qnaProfWriteForm">글쓰기</a>
							</div>
											...
											<c:forEach var="qnaProfNoticeList"
												...
												<tbody>
													<tr role="row" class="odd">
														...
														<td><a
															href="${pageContext.request.contextPath}/admin/qnaProfDetail?board_num=${qnaProfNoticeList.board_num}&board_count=${qnaProfNoticeList.board_count}&pageNo=${pageVO.pageNo}">
																${qnaProfNoticeList.board_title} </a></td>
														<td><fmt:formatDate pattern = "yyyy-MM-dd" value = "${qnaProfNoticeList.board_date}" /></td>
