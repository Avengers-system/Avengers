...
<c:choose>
	<c:when test="${not empty portalNoticeList}">
		<form name="updatePortalBoard" action="portalStudUpdate" method="post"
			...
			첨부파일다운:<a
				href="<%=request.getContextPath()%>/student/helpDesk/main/download?fileName=${portalNoticeList.board_af}&board_num=${board_num}"
				style="text-decoration: underline;">
				${portalNoticeList.board_af} </a><br>

			<sec:authorize access="hasRole('ROLE_WSS')">
          첨부할파일:<input type="file" name="boardtoa"
					value="${portalNoticeList.board_af}">
				<br>
			</sec:authorize>
			...
			<sec:authorize access="hasRole('ROLE_WSS')">
				...
				<a href="portalStudDelete?board_num=${portalNoticeList.board_num}"><input
